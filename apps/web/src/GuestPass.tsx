...
const GuestPass: React.FC = () => {
  ...
  const [status, setStatus] = useState<
    "loading" | "valid" | "redeemed" | "expired" | "error"
  >("loading");
    ...
    try {
      ...
      if (!currentItem) {
        ...
        const roomNumber = String(
          currentItem.room_number ||
            currentItem.amount ||
            currentItem.roomNumber ||
            "",
        );
        const servicesRaw = String(
          currentItem.service_type || currentItem.services || "",
        );
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        const vRes = await fetch(`${API_BASE_URL}/api/data?sheet=vouchers&id=${id}`);
        if (vRes.ok) {
        ...
        const rRes = await fetch(
          `${API_BASE_URL}/api/data?sheet=redemptions&id=${id}`,
        );
        if (rRes.ok) {
      ...
      <div className="w-full max-w-sm bg-white rounded-[2.5rem] overflow-hidden shadow-2xl relative border border-white/10">
        ...
        {status === "expired" && (
          <div className="px-6 -mt-6 relative z-30">
            <div className="bg-red-600 p-8 rounded-[2rem] text-white shadow-xl text-center border-4 border-white animate-pulse">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-1 opacity-90">
                Access Expired
              </p>
        ...
        <div className="bg-[#1a1a1a] p-12 pb-10 text-center relative">
          ...
          <div className="flex flex-col items-center mb-8">
            <div className="text-[#c5a572] font-serif text-5xl leading-none mb-2 font-light">
              Nº1
            </div>
            <div className="text-white font-serif tracking-[0.4em] text-[10px] uppercase font-bold opacity-80 border-t border-white/10 pt-2 px-4">
              Wellness Club
            </div>
          </div>
          ...
          {status === "expired" ? (
            ...
            <>
              <div className="text-center mb-4">
                <p className="text-[11px] text-[#c5a572] font-bold uppercase tracking-widest mb-2">
                  ✨ Your Exclusive Benefits ✨
                </p>
                <p className="text-[9px] text-gray-400">
                  Show this pass to reception to redeem your treatments
                </p>
              </div>
              <div className="bg-white p-4 rounded-2xl shadow-xl border border-gray-100 relative">
                <QRCode
                  value={`${import.meta.env.VITE_APP_URL || window.location.origin}/v/${id}`}
                  size={140}
                />
              </div>
            ...
            <div className="mt-6 w-full bg-green-500/5 rounded-2xl p-4 border border-green-500/10 text-left">
              ...
              <div className="space-y-3">
                {redemptions.map((redeem, idx) => (
                  <div
                    key={idx}
                    className="border-b border-green-500/10 last:border-0 pb-2 last:pb-0"
                  >
                      ...
                      <div className="flex flex-col">
                        ...
                        {redeem.total && (
                          <span className="text-[9px] text-[#c5a572] font-black uppercase tracking-tighter mt-0.5">
                            Total: {Number(redeem.total).toLocaleString()} IDR
                            (Inc. Tax & Service)
                          </span>
                        )}
                      </div>
                      <span className="text-gray-400 text-[9px] whitespace-nowrap pt-0.5 uppercase font-bold tracking-tighter">
                        {new Date(redeem.timestamp).toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                        {redeem.weather && redeem.weather.length < 50 && (
                          <span className="ml-1">• {redeem.weather}</span>
                        )}
                      </span>
                    </div>
                  </div>
